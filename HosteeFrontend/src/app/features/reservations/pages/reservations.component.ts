...
@Component({
  ...
  template: `
    <div class="flex flex-col gap-8 font-sans">
      <!-- Top Filters Banner -->
      <div
        class="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4"
      >
        <!-- Page Header -->
        <app-page-header>
          <!-- Left -->
          ...
          <app-action-button
            ...
            (buttonClicked)="openStepperDrawer()"
          ></app-action-button>
        </app-page-header>
        <!-- Page Header ends -->
      ...
      @if (viewMode() === 'kanban') {
        <div class="grid grid-cols-1 md:grid-cols-5 gap-6 items-start overflow-x-auto pb-4">
          @for (column of kanbanColumns(); track column.status) {
            <div
              class="bg-slate-50 border border-slate-100 rounded-3xl p-4 min-w-[240px] flex flex-col gap-4"
            >
              <!-- Column Header -->
              <div class="flex items-center justify-between px-2">
                ...
                <span
                  class="text-[10px] px-2 py-0.5 bg-slate-200/50 text-slate-600 font-bold rounded-full"
                >
                  {{ column.items.length }}
                </span>
              </div>
                ...
                @for (res of column.items; track res.id) {
                  ...
                  >
                    <div>
                      ...
                      >
                      <h4
                        class="text-xs font-extrabold text-slate-700 font-sans group-hover:text-indigo-600 mt-0.5"
                      >
                        {{ res.guestName }}
                      </h4>
                    ...
                    <div
                      class="flex items-center justify-between border-t border-slate-50 pt-2 text-[10px] text-slate-400 font-bold"
                    >
                  ...
                  <div
                    class="py-12 border border-dashed border-slate-200 rounded-2xl text-center text-[10px] text-slate-400 font-medium"
                  >
                    No reservations
                  </div>
                }
              ...
              <tbody>
                @for (res of reservationsStore.filteredReservations(); track res.id) {
                  ...
                  >
                    ...
                    <td class="px-6 py-4 text-xs font-black text-slate-700 text-right">
                      \${{ res.totalAmount }}
                    </td>
                  </tr>
                }
              ...
              @for (step of [1, 2, 3]; track step) {
                <div class="flex-1 flex items-center gap-1.5">
                  ...
                  <span
                    ...
                    [ngClass]="{ 'text-indigo-600': activeStep() === step }"
                  >
                    {{ step === 1 ? 'Guest' : step === 2 ? 'Room' : 'Payment' }}
                  </span>
                  @if (step < 3) {
                    <div
                      class="flex-1 h-0.5 bg-slate-100"
                      [ngClass]="{ 'bg-indigo-200': activeStep() > step }"
                    ></div>
                  }
                </div>
              }
          ...
          <div class="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
                ...
                <div class="flex flex-col gap-3">
                  @for (gst of guestsStore.guests(); track gst.id) {
                    <div
                      ...
                      [ngClass]="{
                        'border-indigo-500 bg-indigo-50/20': selectedGuestRecord()?.id === gst.id,
                      }"
                    >
                      <div class="flex items-center gap-3">
                        <div
                          class="w-8 h-8 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-500 text-xs font-bold"
                        >
                          {{ gst.name.charAt(0) }}
                        </div>
                        <div>
                          ...
                          <span class="text-[10px] text-slate-400 font-semibold block mt-0.5"
                            >{{ gst.email }} • Tier: {{ gst.loyaltyTier }}</span
                          >
                        </div>
                      </div>
                      <i
                        class="pi pi-check text-xs text-indigo-600 opacity-0"
                        [ngClass]="{ 'opacity-100': selectedGuestRecord()?.id === gst.id }"
                      ></i>
                    </div>
                  }
                </div>
                ...
                <div class="flex flex-col gap-3">
                  @for (rm of vacantRooms(); track rm.id) {
                    <div
                      ...
                      class="p-4 bg-slate-50 hover:bg-slate-100 border border-slate-100 rounded-2xl cursor-pointer flex items-center justify-between transition-all"
                      [ngClass]="{
                        'border-indigo-500 bg-indigo-50/20': selectedRoomRecord()?.id === rm.id,
                      }"
                    >
                      <div class="flex items-center gap-3">
                        <div
                          class="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-500 flex items-center justify-center text-xs font-bold"
                        >
                          {{ rm.number }}
                        </div>
                        <div>
                          ...
                          <span class="text-[10px] text-slate-400 font-semibold block mt-0.5"
                            >Rate: \${{ rm.rate }}/night • Capacity:
                            {{ rm.maxOccupancy }} Guests</span
                          >
                        </div>
                      </div>
                      <i
                        class="pi pi-check text-xs text-indigo-600 opacity-0"
                        [ngClass]="{ 'opacity-100': selectedRoomRecord()?.id === rm.id }"
                      ></i>
                    </div>
                  }
                </div>
            ...
            @if (activeStep() === 3) {
              <div class="flex flex-col gap-6">
                <!-- Stay summary -->
                <div
                  class="bg-indigo-50/40 border border-indigo-100 rounded-2xl p-4 flex flex-col gap-3"
                >
                  <span class="text-[10px] uppercase font-bold text-indigo-600 tracking-wider"
                    >Stay & Tariff Breakdown</span
                  >

                  <div class="grid grid-cols-2 gap-4 border-b border-indigo-100/30 pb-3">
                    <div>
                      <span class="text-[9px] text-slate-400 font-bold block uppercase"
                        >CRM Primary Guest</span
                      >
                    ...
                    <div>
                      ...
                      >
                      <span class="text-xs font-extrabold text-slate-700"
                        >Room {{ selectedRoomRecord()?.number }} ({{
                          selectedRoomRecord()?.type
                        }})</span
                      >
                    </div>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-4 border-b border-indigo-100/30 pb-3">
                    <div class="flex flex-col gap-1">
                      <label class="text-[9px] text-slate-400 font-bold uppercase"
                        >Check In Date</label
                      >
                      <input
                        type="date"
                        [(ngModel)]="bookingForm.checkIn"
                        class="text-xs font-bold bg-white px-2.5 py-1.5 border border-slate-200 rounded-xl"
                      />
                    </div>
                    <div class="flex flex-col gap-1">
                      <label class="text-[9px] text-slate-400 font-bold uppercase"
                ...
                <div class="flex flex-col gap-1.5">
                  <label class="text-[10px] uppercase font-bold text-slate-400"
                    >Special Guest Requests</label
                  >
                  <textarea
                    [(ngModel)]="bookingForm.specialRequests"
                    placeholder="Feather pillows, high floor..."
                    class="w-full text-xs font-bold text-slate-700 bg-slate-50 border border-slate-200 rounded-xl p-3 h-24 outline-none focus:border-indigo-500"
                  ></textarea>
                </div>
              </div>
            }
...
export class ReservationsComponent {
  ...
  get searchQuery(): string {
    return this.reservationsStore.searchQuery();
  }
  set searchQuery(val: string) {
    this.reservationsStore.searchQuery.set(val);
  }
    ...
    return [
      {
        title: 'Inquiry Lead',
        status: 'inquiry',
        items: list.filter((r) => r.status === 'inquiry'),
      },
      {
        title: 'Confirmed Stays',
        status: 'confirmed',
        items: list.filter((r) => r.status === 'confirmed'),
      },
      {
        title: 'Checked In',
        status: 'checked_in',
        items: list.filter((r) => r.status === 'checked_in'),
      },
      {
        title: 'Checked Out',
        status: 'checked_out',
        items: list.filter((r) => r.status === 'checked_out'),
      },
      {
        title: 'Cancelled',
        status: 'cancelled',
        items: list.filter((r) => r.status === 'cancelled'),
      },
    ];
  });
  ...
  selectBooking(res: Reservation): void {
    ...
    if (confirm(`Adjust reservation status for ${res.guestName}?`)) {
      const nextStatus = prompt(
        `Enter new status (inquiry, confirmed, checked_in, checked_out, cancelled):`,
        res.status,
      );
      if (nextStatus) {
